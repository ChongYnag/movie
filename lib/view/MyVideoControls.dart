...
class MyVideoControls extends StatefulWidget {
  String title;

  MyVideoControls(this.title);

  @override
  State<StatefulWidget> createState() {
    return _MyVideoControlsState();
  }
}

class _MyVideoControlsState extends State<MyVideoControls> {
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        onHorizontalDragEnd: (endDetail) {
          if (endDetail.velocity.pixelsPerSecond.dx < 0) {
            chewieController
                .seekTo(_latestValue.position - Duration(seconds: 5));
          } else {
            chewieController
                .seekTo(_latestValue.position + Duration(seconds: 5));
          }
        },
        onVerticalDragEnd: (endDetail) {
          if (endDetail.velocity.pixelsPerSecond.dy < 0) {
            if (_latestValue.volume == null) {
              chewieController.setVolume(_latestVolume ?? 0.5);
            }

            chewieController.setVolume(_latestValue.volume + 0.1);
            print(_latestValue.volume);
          } else {
            if (_latestValue.volume == null) {
              chewieController.setVolume(_latestVolume ?? 0.5);
            }
